              ...
              <tbody>
                {slots.map((slot) => (
                  <tr key={slot.id}>
                    ...
                    <td>{new Date(`1970-01-01T${slot.slot_time}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}</td>
          ...
          <div className={styles.cardWrapper}>
            {slots.map((slot) => (
              <div key={slot.id} className={styles.slotCard}>
                ...
                <p><strong>Time:</strong> {new Date(`1970-01-01T${slot.slot_time}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}</p>
                <p><strong>Type:</strong> {slot.appointment_type}</p>
                <p><strong>Status:</strong> <span className={styles[slot.status]}>{slot.status}</span></p>
              </div>
            ))}
          </div>
        </div>
