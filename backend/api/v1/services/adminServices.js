...
export const getAllPendingSlots = async (req, res) =>{
    try {
        const slots = await pool.query (`SELECT 
    s.id AS id, 
    s.date, 
    s.slot_time, 
    s.appointment_type, 
...
export const getAllSlots = async (req, res) =>{
    try {
        console.log('inside get all slots');
        const slots = await pool.query (`SELECT 
    s.id AS id, 
    s.date, 
    s.slot_time, 
...
export const approveSlot = async (req, res) => {
  ...
  year: "numeric",
    ...
    const rejectedUserIds = rejectedSlotsQuery.rows.map(row => row.user_id);
      ...
      const doctorQuery = await pool.query("SELECT name FROM doctors WHERE id = $1", [doctor_id]);
        if (doctorQuery.rowCount === 0) {
            return res.status(404).json({ error: "Doctor not found" });
        }
        const doctorName = doctorQuery.rows[0].name;

        const userQuery = await pool.query("SELECT email FROM users WHERE id = $1", [user_id]);
