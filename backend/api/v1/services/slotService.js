...
export const getSlots = async (req, res) => {
  ...
  try {
    const bookedSlotsRes = await pool.query(
      `select slot_time from appointments where doctor_id = $1 and date = $2`,
      [doctorId, date.date]
    );
    ...
    const availableMorningSlots = morningSlots.filter(
      (slot) => !formattedSlots.includes(slot)
    );
...
export const bookSlot = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `insert into slots(doctor_id, user_id, date, slot_time, appointment_type, status) VALUES ($1, $2, $3, $4, $5, 'pending')`,
      [doctorId, userId, date, slotTime, appointmentType]
    );
    res.status(200).json({
...
export const slotsByUserId = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM slots WHERE user_id = $1 ORDER BY date ASC, slot_time ASC`,
      [userId]
    );
